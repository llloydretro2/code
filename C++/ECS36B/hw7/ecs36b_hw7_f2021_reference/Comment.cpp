...
(Json::Value *input_json_ptr)
{
  ...
  if ((((*input_json_ptr)["reactions"]).isNull() != true) &&
      ...
      for (j = 0; j < ((*input_json_ptr)["reactions"]["data"]).size(); j++)
	{
	  ...
	  // to be completed by ecs36b_f2021 students for hw6
	  // ************************************************
    Json::Value l_jv_reaction = (*input_json_ptr)["reactions"]["data"][j];
    Reaction *l_reaction_ptr = new Reaction();
    bool rc = l_reaction_ptr->Jdump(&(l_jv_reaction));
    if (rc == true) 
    {
      int flag_rt = 0;
      if (this->reactions != NULL)
      {
        ...
        for (my_it_rt = (*(this->reactions)).begin(); my_it_rt < (*(this->reactions)).end(); my_it_rt++)
        {
          if (((*(*my_it_rt)->author)) == (*(l_reaction_ptr->author))) 
          {
            flag_rt = 1;
          }
        }
